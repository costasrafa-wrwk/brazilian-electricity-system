---
title: "Annex - The Brazilian Electric System Green Transition seen from a Dependency perspective v1.1"
...
This version contains only raw datasets. Some calculations used in the paper are still missing.
...
* *This dataset contains more observations (6857) than there are Brazilian municipalities (5572)*. Since more than one energy distribution company serves certain municipalities, they generate more than one observation.

* *The intention was to identify the energy distribution's controller to highlight the concentration existing in the Brazilian Electricity market*.
...
### Image 1 - Share of GDP by distribution controller

```{r}
brazilian_municipalities_total_gdp <- gdp_mun %>% 
                                        summarise(sum(GDP)) %>% 
                                        pull()

distributors_mun %>% 
  group_by(Company_Controller) %>% 
  filter(Company_Controller!="Unidentified") %>% 
  summarise(GDP=sum(GDP)) %>% 
  mutate(share=round(GDP/brazilian_municipalities_total_gdp*100, 2)) %>% 
  ungroup() %>% 
  mutate(Company_Controller=factor(Company_Controller,
                                   levels=c("Enel",
                                            "Iberdrola",
                                            "State Grid",
                                            ...
                                            "EDP"))) %>%
  ggplot()+
  geom_col(aes(x=Company_Controller, y=share, fill = Company_Controller),
           alpha=0.75)+
  geom_text(aes(x=Company_Controller, y=share, label = share), vjust=2)+
  labs(y="% of GDP", x="")+
  scale_fill_brewer(palette = "Spectral")+
  theme(legend.position = "none", axis.text.x = element_text(angle = 45),
        panel.grid.major.x = element_blank(), panel.grid.minor = element_blank())
```
### Image 2 - Share of Brazilian Population by distribution controller

```{r}
total_brazilian_municipalities_pop <- pop_mun %>% 
                                        summarise(sum(populacao)) %>% 
                                        pull()
...
distributors_mun %>% 
  ...
  mutate(share=round(Population/total_brazilian_municipalities_pop*100, 2)) %>% 
  ungroup() %>% 
  mutate(Company_Controller=factor(Company_Controller,
                                   levels=c("Iberdrola",
                                            "Enel",
...
```
**Source**: Câmara de Comercialização de Energia Elétrica - CCEE, retrieved 28 August 2024 from [https://pda-download.ccee.org.br/7gQCl3h9Ru6ORLv5SyddeA/content](https://pda-download.ccee.org.br/7gQCl3h9Ru6ORLv5SyddeA/content)

### Image 3 - MW genereted by Brazilian Electricity Power Plants in June, 2024, by energy source

```{r}
plant_generation_data %>% 
  group_by(Type_Energy_Source) %>% 
  summarise(MW_Generated_Monthly= round(sum(MW_Generated_Monthly), 2)) %>% 
  ungroup() %>% 
  mutate(Type_Energy_Source=factor(Type_Energy_Source, levels=c(
    "Hydro",
    "Wind",
    "Biomass",
    "Solar",
  ...
  ggplot()+
  geom_col(aes(x=Type_Energy_Source, y=MW_Generated_Monthly, 
               fill = Type_Energy_Source),
           alpha=0.75)+
  labs(x="")+
  scale_fill_brewer(palette = "Spectral")+
  theme(legend.position = "none", axis.text.x = element_text(angle = 45),
        panel.grid.major.x = element_blank(), panel.grid.minor = element_blank())+
  scale_y_continuous("MW generated",
